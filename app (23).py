...
def rect_grad(n, x, y, w, h, c1, c2, angle=5400000):
    """Rectangle with left-to-right gradient."""
    return (f'<p:sp><p:nvSpPr><p:cNvPr id="1" name="{n}"/>'
            f'<p:cNvSpPr><a:spLocks noGrp="1"/></p:cNvSpPr><p:nvPr/></p:nvSpPr>'
            f'<p:spPr><a:xfrm><a:off x="{emu(x)}" y="{emu(y)}"/>'
            f'<a:ext cx="{emu(w)}" cy="{emu(h)}"/></a:xfrm>'
...
def para(text, sz, bold=False, italic=False, col="FFFFFF",
    ...
    ra = ' rtl="1"' if rt else ""
    return (f'<a:p><a:pPr algn="{a2}" indent="0" marL="228600"{ra}>{bx}{sa}</a:pPr>'
            f'<a:r><a:rPr lang="{"ar-SA" if rt else "en-US"}" '
            f'sz="{int(sz*100)}" dirty="0"{b}{i}>'
...
def txbox(n, x, y, w, h, paras):
    return (f'<p:sp><p:nvSpPr><p:cNvPr id="2" name="{n}"/>'
            ...
            f'<p:txBody>'
            f'<a:bodyPr wrap="square" lIns="45720" rIns="45720" tIns="45720" bIns="45720"/>'
...
def slide_num_shape(num, T):
    """Slide number badge bottom-right."""
    return txbox("pg", 12.5, 7.1, 0.7, 0.3,
                 [para(str(num), 10, col=T["su"], alg="r")])

def slide_hdr(title, T, subtitle=""):
    """Professional header: gradient bar + title card + accent line."""
    ar_title = is_ar(title)
    title_align = "r" if ar_title else "l"
    shapes = [
        # Full-width gradient top bar
        rect_grad("topbar", 0, 0, 13.33, 0.18, T["ac"], T.get("ac2", T["ac"])),
        # Title background card
        rect("tcard", 0, 0.18, 13.33, 1.02, T["tb"]),
        # Left accent stripe
        rect("lbar", 0, 0.18, 0.08, 1.02, T["ac"]),
        # Title text
        txbox("tt", 0.22, 0.20, 12.9, 0.98,
              [para(title, 27, bold=True, col=T["tx"], alg=title_align, font=smart_font(title,"Georgia"))]),
    ]
    if subtitle:
        shapes.append(txbox("ts", 0.22, 1.22, 12.9, 0.38,
                            [para(subtitle, 14, italic=True, col=T["su"], alg=title_align)]))
    return shapes

def pptx_native_table(headers, rows, T, x=0.3, y=1.32, w=12.73):
    """Native PPTX table — properly editable in PowerPoint."""
    n    = len(headers)
    cw   = int(emu(w) / n)
    rh_h = emu(0.46)
    rh_d = emu(0.40)
    ...
    def tc_data(val, even):
        ar  = is_ar(val)
        bg  = T["bx"] if even else T["tb"]
        txt = T["su"]
        return (f'<a:tc><a:txBody><a:bodyPr/><a:lstStyle/>'
                f'<a:p><a:pPr algn="{"r" if ar else "ctr"}"/>'
                f'<a:r><a:rPr lang="{"ar-SA" if ar else "en-US"}" sz="1100" dirty="0">'
                f'<a:solidFill><a:srgbClr val="{txt}"/></a:solidFill>'
                f'<a:latin typeface="{"Amiri" if ar else "Calibri"}"/></a:rPr>'
                f'<a:t>{xe(str(val))}</a:t></a:r></a:p></a:txBody>'
                f'<a:tcPr><a:solidFill><a:srgbClr val="{bg}"/></a:solidFill></a:tcPr></a:tc>')

    hdr_cells = "".join(tc_hdr(h) for h in headers)
    body = ""
    for ri, row in enumerate(rows[:20]):
        cells = "".join(tc_data(row[ci] if ci < len(row) else "", ri % 2 == 0) for ci in range(n))
        body += f'<a:tr h="{rh_d}">{cells}</a:tr>'

    total_h = rh_h + rh_d * min(len(rows), 20)
    tbl = (f'<a:tbl><a:tblPr firstRow="1" bandRow="1"/>'
           f'<a:tblGrid>{"".join(f"<a:gridCol w=\"{cw}\"/>" for _ in range(n))}</a:tblGrid>'
           f'<a:tr h="{rh_h}">{hdr_cells}</a:tr>{body}</a:tbl>')
    ...
    return (f'<p:graphicFrame>'
            ...
            f'<a:ext cx="{emu(w)}" cy="{total_h}"/></p:xfrm>'
...
def s_title(s, T):
    ...
    shapes = [
        ...
        # Big gradient left panel
        rect_grad("panel", 0, 0, 5.5, 7.5, T["ac"], T.get("ac2", T["ac"]), angle=16200000),
        # Right bg
        rect("rbg", 5.5, 0, 7.83, 7.5, T["tb"]),
        # Accent corner
        rect("corner", 0, 0, 5.5, 0.18, T.get("ac2", T["ac"])),
        # Decorative circles (visual design)
        rect("d1", 4.2, 5.8, 1.8, 1.8, T["bg"] + "40" if len(T["bg"])==6 else T["tb"]),
        # Logo/icon area
        txbox("ico", 0.3, 0.5, 4.9, 1.0,
              [para("🎓  Kaabe Academic AI", 13, col="FFFFFF" + "CC"[:0], alg="l",
                    bold=False, font="Calibri")]),
        # Main title — big and bold
        txbox("ti", 0.3, 1.6, 4.9, 2.6,
              [para(title, 36 if len(title) < 40 else 28, bold=True,
                    col="FFFFFF", alg=ta,
                    font=smart_font(title, "Georgia"))]),
        # Subtitle
        txbox("su", 5.8, 2.0, 7.2, 1.0,
              [para(sub, 22, col=T["ac"], alg=ta,
                    font=smart_font(sub, "Calibri"))]),
        # Body/author
        txbox("au", 5.8, 3.2, 7.2, 0.7,
              [para(body, 16, italic=True, col=T["su"], alg=ta)]),
        # Bottom accent line
        rect("bln", 5.5, 7.1, 7.83, 0.08, T["ac"]),
        # Slide number
        txbox("pg", 12.5, 7.15, 0.7, 0.3, [para("1", 10, col=T["su"], alg="r")]),
    ]
...
def s_bullets(s, T):
    ...
    sh = slide_hdr(title, T)
    y_start = 1.62 if not s.get("subtitle") else 1.95
    ps = []
    for b in bullets:
        b = str(b)
        bul = "◆" if is_ar(b) else "▸"
        ps.append(para(b, 18, col=T["su"], bul=bul, spc=5))
    sh.append(txbox("bd", 0.32, y_start, 12.7, 5.65, ps))
    sh.append(slide_num_shape(snum, T))
...
def s_lecture(s, T):
    ...
    # Theory section
    if theory:
        ps.append(para("📖 " + ("النظرية" if ar else "Theory"), 13,
                       bold=True, col=T["ac"], alg="r" if ar else "l"))
        ps.append(para(theory, 15, col=T["su"], spc=2))
        ps.append(para(" ", 5, col=T["bg"]))

    # Formula box
    if formula:
        ps.append(para("📐 " + ("الصيغة الرياضية" if ar else "Formula"), 13,
                       bold=True, col=T["ac"], alg="r" if ar else "l"))
        ps.append(para(formula, 17, bold=True, col=T["tx"],
                       font="Courier New", alg="c"))
        if where:
            ps.append(para(where, 13, italic=True, col=T["su"], spc=2))
        ps.append(para(" ", 5, col=T["bg"]))

    # Example
    if example:
        ps.append(para("✏️ " + ("مثال محلول" if ar else "Worked Example"), 13,
                       bold=True, col=T["ac"], alg="r" if ar else "l"))
        ps.append(para(example, 15, col=T["su"], spc=2))
        ps.append(para(" ", 4, col=T["bg"]))

    # Calculation
    if calc:
        ps.append(para("🔢 " + ("خطوات الحل" if ar else "Calculation"), 13,
                       bold=True, col=T["ac"], alg="r" if ar else "l"))
        for line in calc.replace("\\n", "\n").split("\n"):
            if line.strip():
                ps.append(para(line.strip(), 14, col=T.get("mn","93C5FD"),
                               font="Courier New", spc=1))

    sh.append(txbox("bd", 0.32, 1.62, 12.7, 5.65, ps))
    sh.append(slide_num_shape(snum, T))
    return slide_xml(sh, T["bg"])
...
def s_formula(s, T):
    ...
    # Formula hero box — gradient background
    sh.append(rect_grad("fb", 1.0, 1.7, 11.33, 1.5,
                        T["ac"], T.get("ac2", T["ac"]), angle=5400000))
    sh.append(rect("fb2", 1.0, 1.7, 0.12, 1.5, T.get("ac2", T["ac"])))
    sh.append(txbox("fm", 1.2, 1.75, 11.0, 1.4,
                    [para(formula, 32, bold=True, col="FFFFFF",
                          font="Courier New", alg="c")]))

    ps = []
    if where:
        ps.append(para("⚡ " + ("تعريف المتغيرات" if ar else "Variables"), 13,
                       bold=True, col=T["ac"], alg="r" if ar else "l"))
        for part in where.replace(";", ",").split(","):
            part = part.strip()
            if part:
                ps.append(para("  " + part, 14, col=T["su"],
                               bul="•", spc=2))
        ps.append(para(" ", 4, col=T["bg"]))

    if example:
        ps.append(para("✏️ " + ("مثال رقمي" if ar else "Worked Example"), 13,
                       bold=True, col=T["ac"], alg="r" if ar else "l"))
        ps.append(para(example, 15, col=T["su"], spc=3))
        ps.append(para(" ", 4, col=T["bg"]))

    if calc:
        ps.append(para("🔢 " + ("الحل" if ar else "Solution"), 13,
                       bold=True, col=T["ac"], alg="r" if ar else "l"))
        for line in calc.replace("\\n", "\n").split("\n"):
            if line.strip():
                ps.append(para(line.strip(), 15, bold=True,
                               col=T.get("mn","93C5FD"), font="Courier New", spc=1))

    if ps:
        sh.append(txbox("rs", 0.32, 3.35, 12.7, 3.9, ps))
    sh.append(slide_num_shape(snum, T))
    return slide_xml(sh, T["bg"])

def s_two_col(s, T):
    title    = str(s.get("title",""))
    l_title  = str(s.get("left_title",""))
    r_title  = str(s.get("right_title",""))
    l_pts    = s.get("left_points") or []
    r_pts    = s.get("right_points") or []
    snum     = s.get("slide_num","")
    ar       = is_ar(title)
    ...
    # Left card
    sh += [
        rect("lc",  0.25, 1.62, 6.28, 5.6, T["bx"], T["ac"], 0.8),
        rect_grad("lh", 0.25, 1.62, 6.28, 0.55, T["ac"], T.get("ac2",T["ac"])),
        txbox("lt", 0.35, 1.63, 6.08, 0.52,
              [para(l_title, 14, bold=True, col="FFFFFF",
                    alg="r" if is_ar(l_title) else "c")]),
    ]
    lp = [para(str(b), 15, col=T["su"],
               bul="◆" if is_ar(str(b)) else "▸", spc=4)
          for b in l_pts]
    sh.append(txbox("lb", 0.35, 2.25, 6.08, 4.8, lp))

    # Right card
    sh += [
        rect("rc",  6.8, 1.62, 6.28, 5.6, T["bx"], T["ac"], 0.8),
        rect_grad("rh", 6.8, 1.62, 6.28, 0.55, T["ac"], T.get("ac2",T["ac"])),
        txbox("rt", 6.9, 1.63, 6.08, 0.52,
              [para(r_title, 14, bold=True, col="FFFFFF",
                    alg="r" if is_ar(r_title) else "c")]),
    ]
    rp = [para(str(b), 15, col=T["su"],
               bul="◆" if is_ar(str(b)) else "▸", spc=4)
          for b in r_pts]
    sh.append(txbox("rb", 6.9, 2.25, 6.08, 4.8, rp))
    sh.append(slide_num_shape(snum, T))
    return slide_xml(sh, T["bg"])
...
def s_stat(s, T):
    ...
    for i, si in enumerate(stats[:3]):
        ...
        label = str(si.get("label",""))
        ar    = is_ar(label)
        sh += [
            rect(f"sb{i}", xp, 1.62, ww, 4.4, T["bx"], T["ac"], 0.8),
            rect_grad(f"sh{i}", xp, 1.62, ww, 0.14,
                      T["ac"], T.get("ac2",T["ac"])),
            txbox(f"sv{i}", xp+0.1, 2.0, ww-0.2, 1.7,
                  [para(val, 54, bold=True, col=T["ac"],
                        font="Georgia", alg="c")]),
            txbox(f"sl{i}", xp+0.1, 3.75, ww-0.2, 1.0,
                  [para(label, 15, col=T["su"],
                        alg="r" if ar else "c",
                        font=smart_font(label))]),
        ]

    if body:
        sh.append(txbox("bd", 0.32, 6.22, 12.7, 0.98,
                        [para(body, 14, italic=True, col=T["su"])]))
    sh.append(slide_num_shape(snum, T))
    return slide_xml(sh, T["bg"])
...
def s_table(s, T):
    ...
    sh = slide_hdr(title, T)
    if headers and rows:
        sh.append(pptx_native_table(headers, rows, T, x=0.3, y=1.62, w=12.73))
    else:
        sh.append(txbox("nb", 0.32, 2.0, 12.7, 1.0,
...
def s_conclusion(s, T):
    ...
    shapes = [
        rect("bg",  0, 0, 13.33, 7.5, T["bg"]),
        rect_grad("top", 0, 0, 13.33, 0.18, T["ac"], T.get("ac2",T["ac"])),
        rect_grad("bot", 0, 7.32, 13.33, 0.18, T["ac"], T.get("ac2",T["ac"])),
        rect("tcard", 0, 0.18, 13.33, 1.02, T["tb"]),
        rect("lbar",  0, 0.18, 0.08, 1.02, T["ac"]),
        txbox("tt", 0.22, 0.20, 12.9, 0.98,
              [para(title, 28, bold=True, col=T["tx"],
                    alg="r" if ar else "l",
                    font=smart_font(title,"Georgia"))]),
        rect("cbox", 0.25, 1.28, 12.83, 5.85, T["bx"], T["ac"], 0.8),
    ]
    ps = []
    for b in bullets:
        b = str(b)
        bul = "✓"
        ps.append(para(b, 18, col=T["su"], bul=bul, spc=7,
                       font=smart_font(b)))
    shapes.append(txbox("bd", 0.5, 1.52, 12.3, 5.45, ps))
    shapes.append(slide_num_shape(snum, T))
    return slide_xml(shapes, T["bg"])

def build_slide(s, T):
    # Add a complementary accent color to themes that don't have one
    if "ac2" not in T:
        T = dict(T)
        # Derive ac2 from ac — slightly different shade for gradient
        T["ac2"] = T["ac"]
...
def assemble_pptx(xmls):
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("ppt/theme/theme1.xml",
            ...
            '<a:ea typeface="Amiri"/><a:cs typeface="Amiri"/></a:majorFont>'
            '<a:minorFont><a:latin typeface="Calibri"/>'
            '<a:ea typeface="Amiri"/><a:cs typeface="Amiri"/></a:minorFont>'
            '</a:fontScheme>'
...
with st.sidebar:
    ...
    show_ar   = ar_only  # alias used throughout UI
    st.markdown("---")
